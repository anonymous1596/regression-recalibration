#This script generates the base predictions for each seed for the California Housing experiments. We save the predictions for reproducibility (https://pytorch.org/docs/stable/notes/randomness.html)
import torch
...
for seed in seeds:
  ...
  print(valid_data)
  Path(path + '/splits').mkdir(parents = True, exist_ok = True)
  np.savez(path + '/splits/' + str(seed) + '.npz', valid_data = valid_data, test_data = test_data)
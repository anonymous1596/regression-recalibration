...
def filter_valid_data(valid_means, valid_stds, validy):
  ...
  fil = ~(np.logical_or(valid_Fts == 1,  valid_Fts == 0))
...
def CRPS_bin(valid_means, valid_stds, validy, test_means, test_stds):
  ...
  PCEs = np.array([Parallel(n_jobs=10)(delayed(helper_)(i) for i in range(len(n_bins_arr)))])
...
def CRPS_weighted(valid_means, valid_stds, validy, test_means, test_stds):
...
def NLL_bin(valid_means, valid_stds, validy, test_means, test_stds):
